...
<body>
  ...
  <section id="search-section" class="section" style="padding: 20px 0;">
    <div class="container-fluid">
      ...
      <div class="row mt-4">
        ...
        try {
            $stmt = $conn->prepare("SELECT id, Titulo, img FROM articulos_blog");
            $stmt->execute();
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo "<a href='articulo.php?id=" . $row['id'] . "' class='card-link' onclick='logArticleId(" . $row['id'] . ")'>Read more</a>";
                echo "</div>";
                echo "</div>";
                echo "</div>";
            }
  ...
  <!-- Custom script to log article ID -->
  <script>
    function logArticleId(id) {
      console.log("Artículo ID: " + id);
    }
  </script>
</body>
